...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(AuthProvider provider) : super(const AuthStateUninitialized ()) { // AuthBloc ist der Konstruktor // (AP p) ist der Paramter des Konstruktors
    ...
    on<AuthEventSendEmailVerification>((event, emit) async {
      await provider.sendEmailVerification(); // die Methode 'sEV' des 'AuthProvider' wird aufgerufen
    ...
    on<AuthEventRegister>((event, emit) async {
      ...
      try {
        await provider.createUser( // die Methode 'cU' des 'AuthProvider' wird aufgerufen
          email: email,
          password: password,
        );
        await provider.sendEmailVerification();
        emit(const AuthStateNeedsVerification()); // Der Zustand wird in AuthStateNeedsVerification geändert.
      } on Exception catch (e) { //  Falls ein Fehler auftritt, wird der Zustand in AuthStateRegistering geändert, und die Ausnahme wird übergeben.
    ...
    on<AuthEventInitialize>((event, emit) async { //event an dieser Stelle quasi "unnötig" / emit wird genutzt um Infos (den State) zu senden
      ...
      if (user == null) { // Es ist kein Benutzer angemeldet.
        emit( // emit wird genutzt um über Bloc Zustände zu senden
          const AuthStateLoggedOut( // Zustand wird auf 'ASLO' geändert.
            exception: null,
            isLoading: false,
      ...
      } else if (!user.isEmailVerified) { // Falls der User nicht verifiziert ist.
        emit (const AuthStateNeedsVerification());
      } else { // Ist der User verifiziert, dann wird der Zustand auf 'ASLI' gesetzt.
    ...
    on<AuthEventLogIn>((event, emit) async {
      ...
      await Future.delayed(const Duration(seconds: 3)); // Login dauert 3 Sekunden länger (damit man das "Loading..." sehen kann)
